...
try {
  ...
  if ( $result == 'VERIFIED' ) {
    // write a verification record to order history
    if ($paypal_standard->verifyTransaction($_POST, true)) {
      ...
      if ($check = $check_query->fetch_assoc()) {
        if ( $_POST['payment_status'] == "Completed" && $check['orders_status'] == MODULE_PAYMENT_PAYPAL_STANDARD_PENDING_ORDER_STATUS_ID || $check['orders_status'] == MODULE_PAYMENT_PAYPAL_STANDARD_PREPARE_ORDER_STATUS_ID ) {
          ...
          switch ( $_POST['payment_status'] ) {
            case "Completed":
              ...
              break;
            case "Pending":
              $order->info['order_status'] = MODULE_PAYMENT_PAYPAL_STANDARD_PENDING_ORDER_STATUS_ID;
              break;
            case "Denied":
            case "Failed":
              $order->info['order_status'] = MODULE_PAYMENT_PAYPAL_STANDARD_FAILED_ORDER_STATUS_ID;
              break;
          }

          if ( $order->info['order_status'] != $check['orders_status'] ) {
